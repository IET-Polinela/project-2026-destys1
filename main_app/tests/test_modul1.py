...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        # LANGKAH 1: Login sebagai warga biasa
        self.client.login(
            username='warga_test',
            password='Password123!'
        )

        # LANGKAH 2: Akses halaman dashboard admin
        response = self.client.get('/dashboard/')

        # LANGKAH 3: Verifikasi bahwa akses ditolak
        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Warga biasa seharusnya tidak dapat mengakses dashboard admin"
        )