...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            'title': 'Jalan Rusak di Depan Kampus',
            'category': 'Infrastruktur',
            'description': 'Jalan berlubang dan membahayakan pengendara.',
        ...
        # Verifikasi: Data berhasil dibuat dengan HTTP 201 Created
        self.assertEqual(
            response.status_code,
            status.HTTP_201_CREATED,
            "Pembuatan laporan baru seharusnya berhasil"
        )

        # Verifikasi: Reporter otomatis terisi sesuai user login
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        # Verifikasi: Request ditolak dengan HTTP 400
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Laporan tanpa judul seharusnya ditolak"
        )

        # Verifikasi: Error terdapat pada field title
        self.assertIn(
            'title',
            response.data,
            "Field title harus menghasilkan pesan validasi"
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_400_BAD_REQUEST,
            "Laporan tanpa deskripsi seharusnya ditolak"
        )

        # Verifikasi: Error terdapat pada field description
        self.assertIn(
            'description',
            response.data,
            "Field description harus menghasilkan pesan validasi"
        )
