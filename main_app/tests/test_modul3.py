...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        payload = {
            ...
            'description': 'Deskripsi sudah dimodifikasi',
        ...
        # Verifikasi: Sistem menolak perubahan dengan HTTP 403
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan berstatus REPORTED seharusnya tidak dapat dimodifikasi"
        )

        # Verifikasi: Data asli tetap tidak berubah
        self.laporan_reported.refresh_from_db()

        self.assertEqual(
            self.laporan_reported.title,
            'Saluran Air Tersumbat',
            "Data laporan tidak boleh berubah setelah request ditolak"
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        payload = {
            'title': 'AC Diubah Lagi',
            'category': self.laporan_resolved.category,
            'description': 'Isi laporan diubah',
        ...
        self.assertEqual(
            ...
            'AC Rusak di Lab',
            "Data laporan RESOLVED tidak boleh berubah"
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        # Kirim request perubahan status dari REPORTED ke VERIFIED
        response = self.client.post(
            f'/reports/update-status/{self.laporan_reported.pk}/',
            {
                'status': 'VERIFIED'
            }
        )

        # Verifikasi redirect setelah update berhasil
        self.assertEqual(
            response.status_code,
            302,
            "Perubahan status seharusnya berhasil dan melakukan redirect"
        )

        # Verifikasi status di database berubah menjadi VERIFIED
        self.laporan_reported.refresh_from_db()
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        # Coba lompat langsung dari REPORTED ke RESOLVED
        response = self.client.post(
            f'/reports/update-status/{self.laporan_reported.pk}/',
            {
                'status': 'RESOLVED'
        ...
        # Verifikasi redirect karena transisi tidak valid
        self.assertEqual(
            response.status_code,
            302,
            "Request tetap di-redirect meskipun transisi status tidak valid"
        )

        # Verifikasi status laporan tetap REPORTED
        self.laporan_reported.refresh_from_db()

        self.assertEqual(
            self.laporan_reported.status,
            'REPORTED',
            "Status tidak boleh langsung berubah ke RESOLVED"
        )