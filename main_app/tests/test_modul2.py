...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Akses endpoint detail laporan DRAFT milik Warga B
        response = self.client.get(
            f'/api/report/{self.draft_milik_b.id}/'
        )

        # Verifikasi status HTTP 404 Not Found
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik pengguna lain seharusnya tidak dapat diakses"
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # Siapkan payload modifikasi data
        payload = {
            'title': 'Judul Sudah Diretas',
            'category': 'Infrastruktur',
            'description': 'Data telah dimodifikasi',
            'location': 'Lokasi Baru',
            'status': 'DRAFT',
        }

        # Kirim request PUT ke laporan DRAFT milik Warga B
        ...
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik pengguna lain seharusnya tidak dapat dimodifikasi"
        )
        ...
        self.assertEqual(
            ...
            "Data asli di database tidak boleh berubah"
        )